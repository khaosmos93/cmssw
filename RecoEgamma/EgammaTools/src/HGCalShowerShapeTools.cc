...
void HGCalShowerShapeTools::initPerEvent(
...
void HGCalShowerShapeTools::initPerObject(
...
void HGCalShowerShapeTools::setPFRecHitPtrMap(
...
void HGCalShowerShapeTools::setFilteredHitsAndFractions(
    const std::vector <std::pair <DetId, float> > &hitsAndFracs
)
{
    hitsAndFracs_.clear();
...
double HGCalShowerShapeTools::getRvar(
    double cylinderR,
    double energyNorm,
    bool useCellSize
)
...
HGCalShowerShapeTools::ShowerWidths HGCalShowerShapeTools::getPCAwidths(
    double cylinderR
)
{
    if(hitsAndFracs_.empty())
