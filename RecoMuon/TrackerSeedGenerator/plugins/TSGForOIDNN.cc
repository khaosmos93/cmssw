...
TSGForOIDNN::TSGForOIDNN(const edm::ParameterSet& iConfig)
    : src_(consumes<reco::TrackCollection>(iConfig.getParameter<edm::InputTag>("src"))),
      t_estimatorH_(esConsumes<Chi2MeasurementEstimatorBase, TrackingComponentsRecord>(
          edm::ESInputTag("", iConfig.getParameter<std::string>("estimator")))),
      t_magfieldH_(esConsumes<MagneticField, IdealMagneticFieldRecord>()),
      t_propagatorAlongH_(esConsumes<Propagator, TrackingComponentsRecord>(
          edm::ESInputTag("", iConfig.getParameter<std::string>("propagatorName")))),
      t_propagatorOppositeH_(esConsumes<Propagator, TrackingComponentsRecord>(
          edm::ESInputTag("", iConfig.getParameter<std::string>("propagatorName")))),
      ...
      t_SHPOpposite_(esConsumes<Propagator, TrackingComponentsRecord>(
          edm::ESInputTag("", "hltESPSteppingHelixPropagatorOpposite"))),
...
void TSGForOIDNN::produce(edm::StreamID sid, edm::Event& iEvent, edm::EventSetup const& iEventSetup) const {
  ...
  for (unsigned int l2TrackColIndex(0); l2TrackColIndex != l2TrackCol->size(); ++l2TrackColIndex) {
    ...
    LogTrace("TSGForOIDNN") << "TSGForOIDNN::produce: Created TSOSatMuonSystem: " << tsosAtMuonSystem << std::endl;
    ...
    if (getStrategyFromDNN_) {
      ...
      if (std::abs(l2->eta()) < etaSplitForDnn_) {
        // barrel
        std::tie(nHBd, nHLIP, nHLMuS, dnnSuccess_) =
            evaluateDnn(feature_map_, tf_session_barrel_, metadata.get_child("barrel"));
      } else {
        // endcap
        std::tie(nHBd, nHLIP, nHLMuS, dnnSuccess_) =
            evaluateDnn(feature_map_, tf_session_endcap_, metadata.get_child("endcap"));
      }
      if (!dnnSuccess_)
        break;
...
void TSGForOIDNN::makeSeedsFromHits(const GeometricSearchDet& layer,
  ...
  for (std::vector<TrajectoryMeasurement>::const_iterator it = meas.begin(); it != meas.end(); ++it) {
    ...
    LogTrace("TSGForOIDNN") << "TSGForOIDNN::makeSeedsFromHits: Number of seedHits: " << seedHits.size() << std::endl;
...
void TSGForOIDNN::makeSeedsFromHitDoublets(const GeometricSearchDet& layer,
  ...
  for (std::vector<GeometricSearchDet::DetWithState>::iterator idet = dets.begin(); idet != dets.end(); ++idet) {
    ...
    std::vector<TrajectoryMeasurement> mymeas =
        det.fastMeasurements(idet->second, onLayer, propagatorAlong, *estimator);

    // Save valid measurements
    for (std::vector<TrajectoryMeasurement>::const_iterator imea = mymeas.begin(), ed2 = mymeas.end(); imea != ed2;
         ++imea) {
      if (imea->recHit()->isValid())
        meas.push_back(*imea);
    }  // end loop over meas
  }    // end loop over dets
  ...
  for (std::vector<TrajectoryMeasurement>::const_iterator mea = meas.begin(); mea != meas.end(); ++mea) {
    ...
    for (auto compLayer : compLayers) {
      ...
      for (std::vector<GeometricSearchDet::DetWithState>::iterator idet_next = dets_next.begin();
           idet_next != dets_next.end();
           ++idet_next) {
        ...
        std::vector<TrajectoryMeasurement> mymeas_next =
            det.fastMeasurements(idet_next->second, onLayer_next, propagatorAlong, *estimator);

        for (std::vector<TrajectoryMeasurement>::const_iterator imea_next = mymeas_next.begin(),
                                                                ed2 = mymeas_next.end();
             imea_next != ed2;
             ++imea_next) {
          // save valid measurements
          if (imea_next->recHit()->isValid())
            meas_next.push_back(*imea_next);
      ...
      for (std::vector<TrajectoryMeasurement>::const_iterator mea_next = meas_next.begin(); mea_next != meas_next.end();
           ++mea_next) {
        if (nmeas >= max_meas)
...
std::map<std::string, float> TSGForOIDNN::getFeatureMap(reco::TrackRef l2,
...
std::tuple<int, int, int, bool> TSGForOIDNN::evaluateDnn(std::map<std::string, float> feature_map,
                                                         tensorflow::Session* session,
                                                         const pt::ptree& metadata) const {
